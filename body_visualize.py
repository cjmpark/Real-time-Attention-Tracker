...
import numpy as np
import time
from classifiers.body_feature_extraction import BodyOnlyFeatures

extractor = BodyOnlyFeatures()
...
try:
    while True:
        ...
        h, w = frame.shape[:2]
        for name, idx in extractor.key_ub_landmark.items():
            coord = extractor.ub_2dcoord[name]
            if coord[3] > 0.5 and coord[4] > 0.5:
        ...
        nn_dict = extractor.bodyTrack.get_nn_feature()
        rule_dict = extractor.bodyTrack.get_rule_feature()
        stat_dict = extractor.bodyTrack.get_stats()
        pose_coord = extractor.key_ub_landmark
